...
#' \pkg{\link{openxlsx}} and ODS files via the package \pkg{readODS}.
...
add_summary_NER <- function(object, wb, headlines_cs) {
  ...
  df_nobs <- data.frame(Count = c(su$out_of_smp, su$in_smp, su$size_pop,
                                  su$size_smp)
  )
  ...
  writeData(wb = wb, sheet = "summary", x = "Empirical Best Prediction",
            colNames = FALSE)
  addStyle(wb = wb, sheet = "summary", cols = 1, rows = 1, style = title_cs,
           stack = TRUE)
...
add_pointests <- function(object, wb, headlines_cs) {
...
add_precisions <- function(object, MSE, wb, headlines_cs, CV) {
...
add_estims <- function(object, wb, headlines_cs, MSE, CV) {
